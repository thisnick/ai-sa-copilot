...
async def _generate_summary_for_cluster(domain_id: str, cluster_id: str, iteration: int):
  supabase = await create_async_supabase_admin_client()
...
async def _get_cluster_summaries(domain_id: str):
  ...
  for cluster in top_level_clusters:
    summary = await step.run(
      ...
      lambda: _generate_summary_for_cluster(domain_id, cluster['cluster_id'], cluster['iteration'])
    )
    if summary:
