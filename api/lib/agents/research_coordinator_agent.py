...
def create_research_coordinator_agent(settings: Settings) -> AsyncAgent:
  async def instructions(context_variables: ContextVariables):
    ...
    First decide the user's intent.
       * If the user is asking a question without the need to update the runbook, call the answer_question_without_updating_runbook function.
       * If the user is asking you to write or update the runbook:
        * If the runbook has not been written, then follow the runbook writing process below.
        * If the runbook has been written, but the topic is not researched yet, also follow the runbook writing process below.
        * If the topic has been researched, and you are only asked to update the structure or copy based on Already Researched Topics:
          * call update_runbook_outline_without_research to update the runbook outline and structure without conducting research.
          * call update_runbook_section_content_without_research to update the runbook section content without conducting research.
    ...
    3. Once the user indicates there are no more requirements, or you have identified all the key requirements, plan your
       research topics and kick off the research by calling "research_runbook_topics" function, which also takes in a list of *new* research topics.
    ...
    - Do not give answers to the user without doing the research. Use the answer_question_without_updating_runbook call or kick_off_research to
      research the question based on whether the user wants a runbook or just an answer.
  ...
  async def research_runbook_topics(context_variables: ContextVariables, topics: List[ResearchTopic]) -> AsyncResult:
    ...
    the user's *new* questions and requirements. Do not repeat the topics that have already been researched.
    The research agent will thoroughly research your given topic and start writing the runbook. Call this method if
    the user is interested in getting a step-by-step result on how to perform a task.
